...
namespace sjtu {
  ...
  int2048 &int2048::add(const int2048 &x){
    if(op!=x.op){
      if(bigger_abs((*this),x)) *this = minus_abs(*this,x);
      else *this=minus_abs(x,*this);
    }
    else{
  //    printf("add:%d %d\n",a[1],a[2]);
      (*this)=add_abs(*this,x);
  ...
  int2048 add(int2048 x,const int2048 &y){
    if(x.op!=y.op){
  //    printf("a");
      if(bigger_abs(x,y)) return minus_abs(x,y);
      else return minus_abs(y,x);
    }
    else{
  //    printf("b");
      x=add_abs(x,y);
  ...
  int2048 minus(int2048 x, const int2048 &y){
    ...
    //  int2048(y).print();puts("");
      if(bigger_abs(x,y)) x=minus_abs(x,y);
      else{
        x=minus_abs(y,x);
        x.op^=1;
      }
  ...
  void ntt(int2048 &f,int id){
    for(int i=0;i<n;i++)
      if(i<rev[i]) std::swap(f.a[i],f.a[rev[i]]);
    ...
    if(id==-1){
      ...
      for(int i=0;i<n;i++) f.a[i]=(__int128)f.a[i]*inv%inf;
    }
  }
  int2048 operator*(int2048 x, int2048 y){
    ...
    n=1;while(n<=m) n<<=1;
    for(int i=x.a.size();i<n;i++) x.a.push_back(0);
    for(int i=y.a.size();i<n;i++) y.a.push_back(0);
    for(int i=0;i<n;i++) rev[i]=(rev[i>>1]>>1)|((i&1)?(n>>1):0);
    ntt(x,1);ntt(y,1);
    for(int i=0;i<n;i++) x.a[i]=(__int128)x.a[i]*y.a[i]%inf;
    ntt(x,-1);
    while(x.a.size()>m+1) x.a.pop_back();
    for(int i=0;i+1<x.a.size();i++){
      x.a[i+1]+=x.a[i]/bit;
      x.a[i]%=bit;
    }
...
/*    for(int i=0;i<x.a.size()+y.a.size();i++) ans.a.push_back(0);
  ...
  //  printf("Bigger:\n");int2048(y).print();puts("");
  if(x.a.size()<y.a.size()+b) return 0;
  if(x.a.size()>y.a.size()+b) return 1;
  ...
  int2048 move_left(const int2048 &x,int k){
    int2048 ans;ans.a.clear();
    for(int i=1;i<=k;i++) ans.a.push_back(0);
    for(int i=0;i<x.a.size();i++) ans.a.push_back(x.a[i]);
    return ans;
  }
  int2048 move_right(const int2048 &x,int k){
    int2048 ans;ans.a.clear();
    for(int i=k;i<x.a.size();i++) ans.a.push_back(x.a[i]);
    return ans;
  }
  int2048 baoli(const int2048 &x,const int2048 &y){
    if(y==int2048(1)) return x;
  ...
  //  int2048(y).print();puts("");
    ...
    if(x.a.size()<y.a.size()){
      if(o==0) ans.a.push_back(0);
      else ans.a.push_back(1);
      ans.op=o;
      return ans;
    }
  ...
  bool is_zero(const int2048 &x){
    if(x.a.size()==1&&x.a[0]==0) return 1;
    return 0;
  }
  ...
  bool operator < (const int2048 &x,const int2048 &y){
    if(is_zero(x)&&is_zero(y)) return 0;
    if(x.op==1&&y.op==0) return 1;
    if(x.op==0&&y.op==1) return 0;
    if(x.op==1){
  ...
  bool operator>(const int2048 &x, const int2048 &y){
    if(is_zero(x)&&is_zero(y)) return 0;
    if(x.op==1&&y.op==0) return 0;
    if(x.op==0&&y.op==1) return 1;
    if(x.op==1){
      return bigger_abs(y,x);
    }
    return bigger_abs(x,y);
...
1 68 19 61 78 56 86 30 66 46 00 67 51 94 83 04 76 55 83 96 10 81 27 230899809780398974552684228297
*/